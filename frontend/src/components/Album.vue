<template>
    ...
    <form @submit.prevent="isEditing ? updateAlbum() : addAlbum()">
        ...
        <button class="form-button" type="submit">{{ isEditing ? "Update" : "Add" }}</button>
        <button class="form-button" type="button" v-if="isEditing" @click="cancelEdit">Cancel</button>
    </form>
...
<script setup>
    ...
    const albums = ref([]);
        ...
        try {
            ...
            albums.value = data._embedded.albums.map(album => {
  const idMatch = album._links.self.href.match(/\/albums\/(\d+)$/);
  return {
    ...album,
    ...
    const updateAlbum = async () => {
        try {
            const response = await fetch(`/albums/${currentAlbum.value.id}`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(currentAlbum.value),
            });
    ...
    const deleteAlbum = async (id) => {
        try {
            const response = await fetch(`/albums/${id}`, { method: 'DELETE' });
